...
export default class CardView extends Component {
	...
	render() {
		...
		for (const hashtag of hashtags) {
			listViews.push(
				<li>
					<p className="tagname meiryo"><a href={`/server/${server.name}/${hashtag.tagname}`}>#{hashtag.tagname}</a></p>
		...
		return (
			<div className="inside hashtags-container round">
				<div className="content card">
					<h2 className="title">
						<span className="text">ルーム一覧</span>
						<a href={`/hashtag/${server.name}/create`} className="create">作成</a>
					</h2>
