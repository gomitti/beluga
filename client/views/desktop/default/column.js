...
import { ColumnStore, default_options as column_options, default_settings as column_settings } from "../../../stores/column"
...
export class ColumnContainer extends Component {
	...
	equals = (a, b) => {
		assert(is_object(a), "@a must be object")
		assert(is_object(b), "@b must be object")
	...
	restore = () => {
		...
		try {
			...
			assert(is_array(pickles), "@pickles must be array")
			...
			for (let pickle_index = 0; pickle_index < pickles.length; pickle_index++) {
				...
				assert(is_string(target), "@target must be string")
