...
import { EmojiPickerView, map_shortname_fname, map_fname_category } from "../../../../../views/theme/default/desktop/emoji"
...
export default class App extends Component {
    ...
    render() {
        ...
        if (Array.isArray(media_favorites) === false) {
            return null
        }
        if (Array.isArray(media_history) === false) {
            return null
        }
        if (Array.isArray(emoji_favorites) === false) {
            return null
        }
        return (
            <div id="app" className="settings">
                <Head title={`お気に入り / 設定 / ${config.site.name}`} platform={platform} logged_in={logged_in} />
