...
export default class Store {
    ...
    async get(encrypted_id) {
        assert(is_string(encrypted_id), "$encrypted_id must be of type string")
        let session = null
    ...
    async save(session) {
        try {
            ...
            if (session.user_id !== null) {
                if (!!(session.user_id instanceof ObjectID) === false) {
        ...
        const existing = await this.get(session.encrypted_id)
        if (existing !== null) {
        ...
        this.cache[session.encrypted_id] = session
        return true
    }
