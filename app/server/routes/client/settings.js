...
module.exports = (fastify, options, next) => {
    fastify.next("/settings/profile", async (app, req, res) => {
        ...
        if (logged_in_user) {
            pinned_emoji = await model.v1.account.pin.emoji.list(fastify.mongo.db, { "user_id": logged_in_user.id })
        ...
        app.render(req.req, res.res, `/theme/${fastify.theme(req)}/${device}/settings/profile`, {
            csrf_token, profile_image_size, logged_in_user, device, pinned_emoji,
    ...
    fastify.next("/settings/pins", async (app, req, res) => {
        ...
        const pinned_emoji = await model.v1.account.pin.emoji.list(fastify.mongo.db, { "user_id": logged_in_user.id })

        const device = fastify.device(req)
        app.render(req.req, res.res, `/theme/${fastify.theme(req)}/${device}/settings/pins`, {
            csrf_token, logged_in_user, pinned_media, recent_uploads, pinned_emoji, device,
            "platform": fastify.platform(req),
        })
    })
    ...
    fastify.next("/settings/desktop", async (app, req, res) => {
        ...
        app.render(req.req, res.res, `/theme/${fastify.theme(req)}/${device}/settings/desktop`, {
            csrf_token, logged_in_user, device, desktop_settings,
            "platform": fastify.platform(req),
        })
    })
    fastify.next("/settings/access_token", async (app, req, res) => {
        ...
        app.render(req.req, res.res, `/theme/${fastify.theme(req)}/${device}/settings/access_token`, {
            csrf_token, logged_in_user, access_tokens, device,
            "platform": fastify.platform(req),
        })
    })
    fastify.next("/settings/uploads", async (app, req, res) => {
        ...
        const media = await collection.v1.media.list(fastify.mongo.db, { "user_id": logged_in_user.id, "count": 100 })
        const aggregation_result = await model.v1.media.aggregate(fastify.mongo.db, { "user_id": logged_in_user.id })
        const device = fastify.device(req)
        app.render(req.req, res.res, `/theme/${fastify.theme(req)}/${device}/settings/uploads`, {
            csrf_token, logged_in_user, device, media, aggregation_result,
            "platform": fastify.platform(req),
        })
    })
